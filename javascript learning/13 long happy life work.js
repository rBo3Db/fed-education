ancestry = [
  ...
  // var live = [];
   var senteries = {
    ...
    //поиск по матери. мы записываем в пустой объект объекты из массива элемен
   ancestry.forEach(function(person) {
        sentery = Math.ceil(person.died / 100);
        // if(senteries[sentery] != sentery) {
          senteries[sentery] = [];
        // }
        // senteries[sentery] = live.push(person.died - person.born);
   });
   ancestry.forEach(function(person) {
      sentery = Math.ceil(person.died / 100);
        senteries[sentery].push(person.died - person.born);
   });
   
  // var group = ancestry.filter(
  //   function(person) { //удаляем всех матерей без детей

  //       // console.log (sentery);
  //     return ;
  //   }).map(
        
  //      function(person) { //вычитаем из даты рождения ребенка дату рождения матери
  //        return person.died-person.born;
  //      });
  for (var key in senteries) {
    senteries[key] = average(senteries[key]);
  }
  console.log(JSON.stringify(senteries));
