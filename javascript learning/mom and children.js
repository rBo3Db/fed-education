var ancestry =[
  ...
  function average(array) { //вычисление среднего возроста
    function plus(a, b) { return a + b; }
    return array.reduce(plus) / array.length;
  }
  
  var byName = {};//поиск по матери. мы записываем в пустой объект объекты из массива объектов.
  ancestry.forEach(function(person) {
    byName[person.name] = person;
  });
  
  var diff = ancestry.filter(
    function(person) { //удаляем всех матерей без детей в новый массив
      return byName[person.mother] != null;
    }).map(
       function(person) { //вычитаем из даты рождения ребенка дату рождения матери в новый массив
